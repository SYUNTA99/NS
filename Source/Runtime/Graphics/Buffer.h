...
namespace NS::Graphics
{
    ...
    //! @param[in] data        頂点データの先頭。DEFAULT usage では必須
    //! @param[in] vertexCount 頂点の個数
    //! @param[in] stride      頂点 1 個のバイト数
    //! @param[in] usage       バッファの用途。既定は GPU 読み取り専用の DEFAULT
    ...
    //! @param[in] data       index データの先頭。DEFAULT usage では必須
    //! @param[in] indexCount index の個数
    //! @param[in] format     index の型。R16_UINT か R32_UINT
    //! @param[in] usage      バッファの用途。既定は GPU 読み取り専用の DEFAULT
