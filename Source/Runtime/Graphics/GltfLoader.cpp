...
namespace NS::Graphics
{
    namespace
    {
        ...
        void AppendMesh(const cgltf_mesh& mesh, const float world[16], const std::string& path, MeshGeometry& geom)
        {
            for (cgltf_size p = 0; p < mesh.primitives_count; ++p)
            {
                ...
                {
                    NS_LOG_ERROR(Graphics, "LoadGltfMesh: Draco 圧縮 primitive は未対応のため skip (path={})", path);
                    continue;
                }
    ...
    MeshGeometry LoadGltfMesh(const std::string& path)
    {
        ...
        {
            NS_LOG_ERROR(Graphics, "LoadGltfMesh: KHR_draco_mesh_compression は未対応 (path={})", path);
            return geom;
        }

        // node ごとに world 変換して mesh を連結
    ...
    namespace
    {
        ...
        bool BuildSkeletonBones(const cgltf_skin& skin, const std::string& path, std::vector<Bone>& outBones)
        {
            ...
            {
                NS_LOG_ERROR(Graphics, "LoadGltfSkinnedMesh: skin の joint が 0 (path={})", path);
                return false;
            }
            if (skin.joints_count > k_MaxBones)
            {
                NS_LOG_ERROR(Graphics,
            ...
            {
                NS_LOG_ERROR(Graphics, "LoadGltfSkinnedMesh: inverse bind 行列が欠落 (path={})", path);
                return false;
            }
                                    ...
                                    std::vector<std::uint32_t>& indices)
        {
            ...
            if (posAcc == nullptr)
            {
                NS_LOG_ERROR(Graphics, "LoadGltfSkinnedMesh: POSITION が無い (path={})", path);
                return false;
            }
            const cgltf_accessor* uvAcc = FindAttribute(prim, cgltf_attribute_type_texcoord, 0);
            const cgltf_accessor* normalAcc = FindAttribute(prim, cgltf_attribute_type_normal, 0);
            ...
            {
                NS_LOG_ERROR(Graphics, "LoadGltfSkinnedMesh: JOINTS_0 / WEIGHTS_0 が無い (path={})", path);
                return false;
            }
                             ...
                             std::vector<AnimationClip>& outClips)
        {
            for (cgltf_size a = 0; a < model.animations_count; ++a)
            {
                ...
                for (cgltf_size c = 0; c < anim.channels_count; ++c)
                {
                    ...
                    if (boneIndex < 0)
                    {
                        NS_LOG_WARN(Graphics, "glTF animation: target が骨に対応しないため skip (path={})", path);
                        continue;
                    }
    ...
    SkinnedMeshData LoadGltfSkinnedMesh(const std::string& path)
    {
        ...
        if (RequiresDraco(model))
        {
            NS_LOG_ERROR(Graphics, "LoadGltfSkinnedMesh: KHR_draco_mesh_compression は未対応 (path={})", path);
            return data;
        }
        ...
        {
            NS_LOG_ERROR(Graphics, "LoadGltfSkinnedMesh: skin 付き mesh node が無い (path={})", path);
            return data;
        }
        ...
        if (vertices.empty())
        {
            NS_LOG_ERROR(Graphics, "LoadGltfSkinnedMesh: 有効な skinned 三角形ジオメトリが無い (path={})", path);
            return data;
        }

        // bone 並べ替えに合わせて頂点の joint index を張り替える
    ...
    AnimationSource LoadGltfAnimationSource(const std::string& path)
    {
        ...
        {
            NS_LOG_ERROR(Graphics, "LoadGltfAnimationSource: animation が無い (path={})", path);
            return source;
        }
