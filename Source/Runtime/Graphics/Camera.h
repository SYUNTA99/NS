...
namespace NS::Graphics
{
    ...
    private:
        NS::Math::Vector3 m_position; ///< カメラ位置
        NS::Math::Vector3 m_target;   ///< 注視点
        NS::Math::Vector3 m_up;       ///< アップベクトル
        NS::Math::Radians m_fovY;     ///< 垂直視野角
        float m_aspect;               ///< アスペクト比
        float m_near;                 ///< ニアクリップ距離
        float m_far;                  ///< ファークリップ距離

        mutable NS::Math::Matrix m_view;       ///< ビュー行列のキャッシュ
        mutable NS::Math::Matrix m_projection; ///< プロジェクション行列のキャッシュ
