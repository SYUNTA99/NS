...
namespace NS::Core
{
    /// @brief 複数フレームにまたがる演出手順の台本。co_await で待ちを挟みつつ上から下へ書く
    /// @details SequenceRunner が所有と駆動を持つ。台本の中で待ちをまたいで生ポインタを持たないこと
    ...
    public:
        struct promise_type
        {
            ...
            Sequence get_return_object() { return Sequence{std::coroutine_handle<promise_type>::from_promise(*this)}; }
        ...
        Sequence() = default;
        explicit Sequence(Handle handle) noexcept : m_handle(handle) {}
        ~Sequence()
        {
            if (m_handle)
                m_handle.destroy();
        ...
        Sequence(Sequence&& other) noexcept : m_handle(std::exchange(other.m_handle, {})) {}
        ...
        /// 所有を手放して生ハンドルを返す。SequenceRunner::Start だけが使う
    ...
    struct WaitSeconds
    {
        ...
        void await_suspend(std::coroutine_handle<Sequence::promise_type> handle) const
        {
    ...
    struct WaitUntil
    {
        ...
        void await_suspend(std::coroutine_handle<Sequence::promise_type> handle)
        {
    ...
    struct WaitNextFrame
    {
        ...
        void await_suspend(std::coroutine_handle<Sequence::promise_type> handle) const noexcept
        {
    ...
    public:
        ...
        {
            Sequence::Handle handle = sequence.Release();
            if (!handle)
                return;
            handle.resume();
            if (handle.done())
        ...
        void Tick(float dt)
        {
            ...
            {
                Sequence::Handle handle = m_handles[i];
                Sequence::promise_type& promise = handle.promise();
        ...
        {
            for (Sequence::Handle handle : m_handles)
                handle.destroy();
            m_handles.clear();
        }
    ...
    private:
        std::vector<Sequence::Handle> m_handles; // 進行中の台本。done になったら即座に外す
    };
