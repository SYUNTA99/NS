...
namespace NS::Core
{
    ...
    std::wstring StringUtils::WideFromUtf8(std::string_view utf8)
    {
        ...
        if (written != dstLen)
        {
            NS_LOG_ERROR(Core, "StringUtils::WideFromUtf8 conversion incomplete (written={}, expected={})", written, dstLen);
            return {};
        }
        return result;
    }
    ...
    std::string StringUtils::Utf8FromWide(std::wstring_view wide)
    {
        ...
        const int written = ::WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, wide.data(), srcLen, result.data(), dstLen, nullptr, nullptr);

        if (written != dstLen)
        {
            NS_LOG_ERROR(Core, "StringUtils::Utf8FromWide conversion incomplete (written={}, expected={})", written, dstLen);
            return {};
        }
        return result;
    }
