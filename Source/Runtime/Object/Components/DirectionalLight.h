...
namespace NS::Obj
{
    ...
    public:
        ...
        [[nodiscard]] const NS::Core::Vector3& Direction() const noexcept { return m_direction; }
        //! 平行光の色。1.0 超の HDR 値も許容する
        [[nodiscard]] const NS::Core::Vector3& Color() const noexcept { return m_color; }
        //! 空側の環境光。上を向いた面へ回る
        [[nodiscard]] const NS::Core::Vector3& Ambient() const noexcept { return m_ambient; }
        //! 地面側の環境光。下を向いた面へ回る
        [[nodiscard]] const NS::Core::Vector3& Ground() const noexcept { return m_ground; }
        ...
        NS_REFLECT_BEGIN(DirectionalLight, Component)
    ...
    private:
        NS::Core::Vector3 m_direction{-0.3f, -1.0f, -0.2f}; // 平行光の向き
        NS::Core::Vector3 m_color{1.0f, 1.0f, 1.0f};        // 平行光の色
        NS::Core::Vector3 m_ambient{0.30f, 0.34f, 0.40f};   // 空側の環境光
