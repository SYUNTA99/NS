...
///
/// @details Window / Renderer / Input と仮実装の Audio をサブシステムとして所有し破棄まで管理する
/// `Run()` で Init → MainLoop → Shutdown を順に実行する。Layer 群は WinMain 段階で
/// AddLayer / AddOverlay で積み、iterator が無効化されるため Run ループ内では追加/削除しない
///
/// s_instance を単一保持し assert で多重起動を禁止する。構築失敗時は `IsValid() == false` を返し、
/// `Run` は -1 で即終了。`Get` / `Quit` は未構築時に例外を投げず nullptr を返す
...
namespace NS::App
{
    ...
    /// 描画のプロジェクト既定値は RendererDesc 型の renderer.settings 側で設定する
    ...
    public:
        ...
        /// regular layer を overlay より前へ追加する。ループ中は iterator が無効化されるので Run の前に呼ぶこと
        void AddLayer(std::unique_ptr<NS::App::Layer> layer);

        /// overlay layer を regular より後ろへ追加し OnRender を最後に回す。Run の前に呼ぶこと
