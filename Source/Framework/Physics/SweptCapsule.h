...
/// @file SweptCapsule.h
/// @brief NS::Physics — 丸い自由配置物用の Capsule vs Sphere / Capsule の TOI 計算
///
/// @details プレイヤー capsule の芯線分を、 相手の球または capsule を半径 R = 両半径和 へ膨らませた
...
{
    /// @brief Capsule が motion だけ移動した時の Sphere との最初の接触を、 解析解で端点近似なしに返す
    /// @param capsule   start 位置の入力 Capsule
    /// @param motion    1 frame の変位ベクトル
    /// @param sphere    ターゲット球
    /// @param outToi    [0,1] の接触時刻、 no hit なら 1.0
    /// @param outNormal world の球表面外向きの接触法線、 no hit なら zero
    ...
    /// @param outNormal world の相手 capsule 表面外向きの接触法線、 no hit なら zero
    /// @retresult true = 接触あり / false = no hit
