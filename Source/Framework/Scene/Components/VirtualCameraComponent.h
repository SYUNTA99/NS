...
/// 投影設定をまとめた desired pose を返すだけ。`CameraBrainComponent` が登録済みの
/// vcam から最高優先度の active なものを選び、その pose を 1 個の実 CameraComponent へ書く
/// fixed-step の状態更新は `OnUpdate` で行い、最終姿勢は `EvaluatePose` で返す。follow 系は
...
namespace NS::Scene
{
    ...
    {
    public:
        /// tick 順は Camera 帯で派生が TickPriority::Camera を渡す。vcam 選択用の優先度は別で SetVcamPriority
        /// が設定する
