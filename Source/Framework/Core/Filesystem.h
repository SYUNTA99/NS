...
/// 例外は外に伝播させず、 失敗時は内部で `NS_LOG_ERROR` を出す
...
namespace NS::Core
{
    ...
    public:
        ...
        /// `dir` 直下の通常ファイルを列挙する。`extension` で先頭ドット込みの拡張子絞り込み可
        ...
        /// @details `NS_SHIPPING` の出荷ビルドは exe と同階層、 開発ビルドは `premake5.lua` / `.git` を
