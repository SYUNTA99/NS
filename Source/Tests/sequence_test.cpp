#include "Runtime/Core/Sequence.h"

#include <gtest/gtest.h>

#include <string>
#include <vector>

/// シーケンス実行器の単体検証。待ちの明け方・破棄・複数本の独立を dt 駆動で確かめる

namespace
{
    NS::Core::Sequence WaitSecondsScript(std::vector<std::string>& log, float seconds)
    {
        log.push_back("before");
        co_await NS::Core::WaitSeconds{seconds};
        log.push_back("after");
    }

    NS::Core::Sequence WaitGateScript(std::vector<std::string>& log, const bool& gate)
    {
        log.push_back("before");
        co_await NS::Core::WaitUntil{[&gate] { return gate; }};
        log.push_back("after");
    }

    NS::Core::Sequence NextFrameScript(std::vector<std::string>& log)
    {
        log.push_back("before");
        co_await NS::Core::WaitNextFrame{};
        log.push_back("after");
    }

    NS::Core::Sequence SynchronousScript(std::vector<std::string>& log)
    {
        log.push_back("only");
        co_return;
    }

    // シーケンスフレームの破棄で立つフラグ。CancelAll が中の破棄処理を走らせる証拠に使う
    struct DtorFlag
    {
        bool* flag = nullptr;
        ~DtorFlag()
        {
            if (flag != nullptr)
                *flag = true;
        }
    };

    NS::Core::Sequence HoldRaiiScript(std::vector<std::string>& log, bool& destroyed)
    {
        DtorFlag raii{&destroyed};
        log.push_back("before");
        co_await NS::Core::WaitUntil{[] { return false; }};
        log.push_back("after");
    }
} // namespace

TEST(SequenceTest, StartRunsBodyUntilFirstWait)
{
    std::vector<std::string> log;
    NS::Core::SequenceRunner runner;

    runner.Start(WaitSecondsScript(log, 1.0f));

    EXPECT_EQ(log, (std::vector<std::string>{"before"}));
    EXPECT_TRUE(runner.IsRunning());
}

TEST(SequenceTest, WaitSecondsResumesAfterAccumulatedTime)
{
    std::vector<std::string> log;
    NS::Core::SequenceRunner runner;
    runner.Start(WaitSecondsScript(log, 0.05f));

    runner.Tick(0.02f);
    runner.Tick(0.02f);
    EXPECT_TRUE(runner.IsRunning());
    EXPECT_EQ(log.size(), 1u);

    runner.Tick(0.02f);
    EXPECT_FALSE(runner.IsRunning());
    EXPECT_EQ(log, (std::vector<std::string>{"before", "after"}));
}

TEST(SequenceTest, WaitUntilResumesWhenConditionTurnsTrue)
{
    std::vector<std::string> log;
    bool gate = false;
    NS::Core::SequenceRunner runner;
    runner.Start(WaitGateScript(log, gate));

    runner.Tick(1.0f);
    runner.Tick(1.0f);
    EXPECT_TRUE(runner.IsRunning());

    gate = true;
    runner.Tick(1.0f);
    EXPECT_FALSE(runner.IsRunning());
    EXPECT_EQ(log, (std::vector<std::string>{"before", "after"}));
}

TEST(SequenceTest, WaitUntilAlreadyTrueContinuesWithoutSuspending)
{
    std::vector<std::string> log;
    const bool gate = true;
    NS::Core::SequenceRunner runner;

    // 最初から真の条件は待たず、シーケンスは Start の中で最後まで走り切る
    runner.Start(WaitGateScript(log, gate));

    EXPECT_FALSE(runner.IsRunning());
    EXPECT_EQ(log, (std::vector<std::string>{"before", "after"}));
}

TEST(SequenceTest, NextFrameResumesOnFollowingTick)
{
    std::vector<std::string> log;
    NS::Core::SequenceRunner runner;
    runner.Start(NextFrameScript(log));
    EXPECT_TRUE(runner.IsRunning());

    runner.Tick(0.0f);
    EXPECT_FALSE(runner.IsRunning());
    EXPECT_EQ(log, (std::vector<std::string>{"before", "after"}));
}

TEST(SequenceTest, SynchronousScriptFinishesAtStart)
{
    std::vector<std::string> log;
    NS::Core::SequenceRunner runner;
    runner.Start(SynchronousScript(log));

    EXPECT_FALSE(runner.IsRunning());
    EXPECT_EQ(log, (std::vector<std::string>{"only"}));
}

TEST(SequenceTest, CancelAllDestroysFramesWithoutResuming)
{
    std::vector<std::string> log;
    bool destroyed = false;
    NS::Core::SequenceRunner runner;
    runner.Start(HoldRaiiScript(log, destroyed));
    runner.Tick(1.0f);
    ASSERT_TRUE(runner.IsRunning());

    runner.CancelAll();

    // 続きは走らないが、シーケンスフレーム内の破棄処理は走る
    EXPECT_FALSE(runner.IsRunning());
    EXPECT_TRUE(destroyed);
    EXPECT_EQ(log, (std::vector<std::string>{"before"}));
}

TEST(SequenceTest, MultipleScriptsRunIndependently)
{
    std::vector<std::string> logA;
    std::vector<std::string> logB;
    NS::Core::SequenceRunner runner;
    runner.Start(WaitSecondsScript(logA, 0.01f));
    runner.Start(WaitSecondsScript(logB, 0.03f));

    runner.Tick(0.02f);
    EXPECT_EQ(logA.size(), 2u);
    EXPECT_EQ(logB.size(), 1u);
    EXPECT_TRUE(runner.IsRunning());

    runner.Tick(0.02f);
    EXPECT_EQ(logB.size(), 2u);
    EXPECT_FALSE(runner.IsRunning());
}
