...
namespace
{
    ...
    TEST(AutoTileThemeTest, AllBitmasksMapToValidSlice)
    {
        ...
        for (std::uint16_t themeIdx = 0; themeIdx < static_cast<std::uint16_t>(ThemeId::Count); ++themeIdx)
        {
            ...
            {
                const std::uint16_t slice = NS::Game::Blocks::LookupTextureSlice(
                    theme, static_cast<std::uint8_t>(mask), NS::Game::Blocks::kBlockIdSolid);
    ...
    TEST(AutoTileThemeTest, OutOfRangeThemeFallsBackToGrassSlice)
    {
        ...
        const std::uint16_t grassSlice =
            NS::Game::Blocks::LookupTextureSlice(ThemeId::Grass, 0u, NS::Game::Blocks::kBlockIdSolid);
        const std::uint16_t outOfRangeSlice =
            NS::Game::Blocks::LookupTextureSlice(static_cast<ThemeId>(99), 0u, NS::Game::Blocks::kBlockIdSolid);
    ...
    TEST(AutoTileThemeTest, OutOfRangeMaskFallsBackToSliceZero)
    {
        ...
        const std::uint16_t slice = NS::Game::Blocks::LookupTextureSlice(
            ThemeId::Grass, static_cast<std::uint8_t>(255), NS::Game::Blocks::kBlockIdSolid);
        EXPECT_EQ(slice, 0u);
    }
    ...
    TEST(AutoTileThemeTest, ThemesUseDifferentBaseSlices)
    {
        ...
        const std::uint16_t caveSlice =
            NS::Game::Blocks::LookupTextureSlice(ThemeId::Cave, 0u, NS::Game::Blocks::kBlockIdSolid);
