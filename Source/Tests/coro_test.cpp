...
namespace
{
    NS::Core::Coro WaitSecondsScript(std::vector<std::string>& log, float seconds)
    {
    ...
    NS::Core::Coro WaitGateScript(std::vector<std::string>& log, const bool& gate)
    {
        log.push_back("before");
    ...
    NS::Core::Coro NextFrameScript(std::vector<std::string>& log)
    {
        log.push_back("before");
    ...
    NS::Core::Coro SynchronousScript(std::vector<std::string>& log)
    {
        log.push_back("only");
        co_return;
    }
    ...
    NS::Core::Coro HoldRaiiScript(std::vector<std::string>& log, bool& destroyed)
    {
...
TEST(CoroTest, StartRunsBodyUntilFirstWait)
{
...
TEST(CoroTest, WaitSecondsResumesAfterAccumulatedTime)
{
    std::vector<std::string> log;
    NS::Core::CoroRunner runner;
...
TEST(CoroTest, WaitUntilResumesWhenConditionTurnsTrue)
{
    std::vector<std::string> log;
    bool gate = false;
    NS::Core::CoroRunner runner;
...
TEST(CoroTest, WaitUntilAlreadyTrueContinuesWithoutSuspending)
{
    std::vector<std::string> log;
...
TEST(CoroTest, NextFrameResumesOnFollowingTick)
{
    std::vector<std::string> log;
    NS::Core::CoroRunner runner;
...
TEST(CoroTest, SynchronousScriptFinishesAtStart)
{
    std::vector<std::string> log;
    NS::Core::CoroRunner runner;
...
TEST(CoroTest, CancelAllDestroysFramesWithoutResuming)
{
    std::vector<std::string> log;
...
TEST(CoroTest, MultipleScriptsRunIndependently)
{
