...
TEST(ObjectIdTest, FindObjectIndexByIdReturnsNoIndexForUnknownOrUnset)
{
    ...
    EXPECT_EQ(SceneNs::FindObjectIndexById(level, 9999u), SceneNs::kNoObjectIndex);

    // 0 は「object 無し」の番兵なので、未割当 id 0 の実体が混ざっていても引かない
    level.objects.push_back(SceneNs::ObjectData{});
    EXPECT_EQ(SceneNs::FindObjectIndexById(level, SceneNs::kNoObjectId), SceneNs::kNoObjectIndex);
}
...
TEST(ObjectIdTest, AddObjectCommandAssignsIdAndRedoReusesIt)
{
    ...
    EditorNs::AddObjectCommand cmd(SceneNs::ObjectData{});
    cmd.Do(level);
...
TEST(ObjectIdTest, ObjectRefFieldSurvivesJsonRoundTrip)
{
    ...
    comp.fields.push_back(SceneNs::FieldValue{"Target", NS::Scene::ObjectRef{targetId}});
    ...
    const auto* field = SceneNs::FindField(restored.objects[1].components[0], "Target");
...
TEST(ObjectIdTest, PruneDanglingObjectRefsKeepsValidAndCountsPruned)
{
    ...
    SceneNs::ComponentData comp;
    comp.typeName = "FakeFollowComponent";
    comp.fields.push_back(SceneNs::FieldValue{"Valid", NS::Scene::ObjectRef{validId}});
    comp.fields.push_back(SceneNs::FieldValue{"Dangling", NS::Scene::ObjectRef{12345u}});
    comp.fields.push_back(SceneNs::FieldValue{"Unset", NS::Scene::ObjectRef{}});
    level.objects[0].components.push_back(std::move(comp));
