...
namespace
{
    SceneNs::ObjectData PlaceOneAndTake(EditorNs::PlaceCommand&& cmd) noexcept
    {
        SceneNs::SceneData lv;
        cmd.Do(lv);
        return lv.objects.empty() ? SceneNs::ObjectData{} : lv.objects.front();
    }
} // namespace
...
TEST(PlaceCommandTest, TemplateClonePlacesPrototypeWithBakedTransform)
{
    ...
    SceneNs::ObjectData expected = tmpl.prototype;
    ...
    const SceneNs::ObjectData cloned =
...
TEST(PlaceCommandTest, AllPaletteSlotsClonePlacesPrototype)
{
    ...
    {
        SceneNs::ObjectData expected = slot.prototype;
        expected.positionX = static_cast<float>(cx);
        expected.positionY = static_cast<float>(cy);
        expected.positionZ = static_cast<float>(cz);
