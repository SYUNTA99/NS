...
TEST(NsCoreStringUtils, RoundTripAscii)
{
    ...
    const auto wide = NS::Platform::StringUtils::WideFromUtf8(original);
    const auto back = NS::Platform::StringUtils::Utf8FromWide(wide);
...
{
    const auto wide = NS::Platform::StringUtils::WideFromUtf8("");
    EXPECT_TRUE(wide.empty());

    const auto back = NS::Platform::StringUtils::Utf8FromWide(L"");
...
TEST_F(StringUtilsLoggerTest, InvalidUtf8ReturnsEmpty)
{
    ...
    const auto wide = NS::Platform::StringUtils::WideFromUtf8(invalid);
    EXPECT_TRUE(wide.empty());
}
