...
TEST(NsCoreStringUtils, RoundTripAscii)
{
    ...
    const std::wstring wide = NS::Platform::StringUtils::WideFromUtf8(original);
    const std::string back = NS::Platform::StringUtils::Utf8FromWide(wide);
...
{
    const std::wstring wide = NS::Platform::StringUtils::WideFromUtf8("");
    EXPECT_TRUE(wide.empty());

    const std::string back = NS::Platform::StringUtils::Utf8FromWide(L"");
...
TEST_F(StringUtilsLoggerTest, InvalidUtf8ReturnsEmpty)
{
    ...
    const std::wstring wide = NS::Platform::StringUtils::WideFromUtf8(invalid);
    EXPECT_TRUE(wide.empty());
}
