...
namespace
{
    ...
    NS::Core::Ray MakeAxisProbeRayZ(float px, float startZ)
    {
        return NS::Core::Ray{NS::Core::Vector3{px, 0.0f, startZ + 5.0f}, NS::Core::Vector3{0.0f, 0.0f, -1.0f}};
    }
    ...
    TEST(GizmoEditorWorldDragToAngle, GrabXDragToYAroundZIsQuarterTurn)
    {
        ...
        const float angle = GizmoEditor::WorldDragToAngle(NS::Core::Vector3{0.0f, 0.0f, 0.0f},
                                                          GizmoAxis::Z,
                                                          NS::Core::Quaternion::Identity,
                                                          vp,
                                                          viewport,
                                                          NS::Core::Vector2{600.0f, 300.0f},
                                                          NS::Core::Vector2{400.0f, 150.0f});
    ...
    TEST(GizmoEditorWorldDragToAngle, NonAxisReturnsZero)
    {
        const NS::Core::Matrix vp;
        const NS::Core::Size2D viewport{800, 600};
        EXPECT_NEAR(GizmoEditor::WorldDragToAngle(NS::Core::Vector3{0.0f, 0.0f, 0.0f},
                                                  GizmoAxis::None,
                                                  NS::Core::Quaternion::Identity,
                                                  vp,
                                                  viewport,
    ...
    {
        const NS::Core::Vector3 start{1.0f, 1.0f, 1.0f};
        const NS::Core::Vector3 result = GizmoEditor::ComputeScale(start, GizmoAxis::X, 0.5f, false);
    ...
    {
        const NS::Core::Vector3 start{1.0f, 2.0f, 3.0f};
        const NS::Core::Vector3 result = GizmoEditor::ComputeScale(start, GizmoAxis::Uniform, 0.5f, false);
        EXPECT_NEAR(result.x, 1.5f, 1.0e-5f);
    ...
    {
        const NS::Core::Vector3 start{1.0f, 1.0f, 1.0f};
        const NS::Core::Vector3 result = GizmoEditor::ComputeScale(start, GizmoAxis::X, -10.0f, false);
    ...
    {
        const NS::Core::Vector3 start{1.3f, 2.7f, 0.4f};
        const NS::Core::Vector3 result = GizmoEditor::ComputeScale(start, GizmoAxis::None, 0.5f, true);
    ...
    TEST(GizmoEditor, ComputeScaleSnapRoundsToQuarterStep)
    {
        ...
        const NS::Core::Vector3 start{1.0f, 1.0f, 1.0f};
        const NS::Core::Vector3 result = GizmoEditor::ComputeScale(start, GizmoAxis::Uniform, 0.6f, true);
        EXPECT_NEAR(result.x, 1.5f, 1.0e-5f);
    ...
    TEST(GizmoEditor, ComputeScaleSnapToZeroRescuedByMinimum)
    {
        ...
        const NS::Core::Vector3 start{1.0f, 1.0f, 1.0f};
        const NS::Core::Vector3 result = GizmoEditor::ComputeScale(start, GizmoAxis::X, -0.9f, true);
        EXPECT_NEAR(result.x, 0.01f, 1.0e-5f);
        EXPECT_NEAR(result.y, 1.0f, 1.0e-5f);
        EXPECT_NEAR(result.z, 1.0f, 1.0e-5f);
    }
    ...
    TEST(GizmoEditor, ScreenDragToScaleAmountProjectsOntoAxis)
    {
        ...
        const float amount =
            GizmoEditor::ScreenDragToScaleAmount(NS::Core::Vector2{1.0f, 0.0f}, NS::Core::Vector2{100.0f, 0.0f});
    ...
    TEST(GizmoEditor, ScreenDragToScaleAmountUsesNormalizedAxis)
    {
        ...
        const float amount =
            GizmoEditor::ScreenDragToScaleAmount(NS::Core::Vector2{3.0f, 4.0f}, NS::Core::Vector2{3.0f, 4.0f});
    ...
    TEST(GizmoEditor, ScreenDragToScaleAmountPerpendicularDragIsZero)
    {
        // 軸 +X に対し縦ドラッグは射影 0
        const float amount =
            GizmoEditor::ScreenDragToScaleAmount(NS::Core::Vector2{1.0f, 0.0f}, NS::Core::Vector2{0.0f, 80.0f});
    ...
    TEST(GizmoEditor, ScreenDragToScaleAmountFallbackNegativeXShrinks)
    {
        ...
        const float amount =
            GizmoEditor::ScreenDragToScaleAmount(NS::Core::Vector2{0.0f, 0.0f}, NS::Core::Vector2{-6.0f, 8.0f});
    ...
    TEST(GizmoEditorDrag, MoveDragChangesOnlyAxis)
    {
        ...
        const NS::Core::Matrix vp;
        const NS::Core::Size2D viewport{100, 100};
        gizmo.ApplyDragForTest(
            vp, viewport, GizmoAxis::X, NS::Core::Vector2{50.0f, 50.0f}, NS::Core::Vector2{60.0f, 50.0f});
    ...
    TEST(GizmoEditorDrag, MoveFollowsLocalAxisWhenRotated)
    {
        ...
        const NS::Core::Size2D viewport{100, 100};
        gizmo.ApplyDragForTest(
            vp, viewport, GizmoAxis::X, NS::Core::Vector2{50.0f, 50.0f}, NS::Core::Vector2{50.0f, 40.0f});
    ...
    TEST(GizmoEditorDrag, NoOpDragLeavesPositionUnchanged)
    {
        ...
        const NS::Core::Size2D viewport{100, 100};
        gizmo.ApplyDragForTest(
            vp, viewport, GizmoAxis::X, NS::Core::Vector2{50.0f, 50.0f}, NS::Core::Vector2{50.0f, 50.0f});
    ...
    TEST(GizmoEditorDrag, RotateDragChangesRotation)
    {
        ...
        const NS::Core::Size2D viewport{100, 100};
        gizmo.ApplyDragForTest(
            vp, viewport, GizmoAxis::Z, NS::Core::Vector2{60.0f, 50.0f}, NS::Core::Vector2{50.0f, 60.0f});

        const NS::Core::Quaternion after = t.Rotation();
    ...
    TEST(GizmoEditorDrag, UniformScaleDragGrowsAllAxesUniformly)
    {
        ...
        const NS::Core::Size2D viewport{100, 100};
        gizmo.ApplyDragForTest(
            vp, viewport, GizmoAxis::Uniform, NS::Core::Vector2{50.0f, 50.0f}, NS::Core::Vector2{70.0f, 50.0f});
    ...
    NS::Core::Matrix MakeOrbitViewProjection(const NS::Core::Vector3& eye, const NS::Core::Vector3& target)
    {
        NS::Core::Matrix view;
    ...
    bool ProjectPoint(const NS::Core::Matrix& vp, const NS::Core::Vector3& world, NS::Core::Vector2& out)
    {
        const NS::Core::Vector4 clip =
            NS::Core::Vector4::Transform(NS::Core::Vector4{world.x, world.y, world.z, 1.0f}, vp);
        if (clip.w <= 1.0e-4f)
            return false;
    ...
    void ExpectArrowGrabAndDrag(GizmoAxis axis, const NS::Core::Vector3& axisDir)
    {
        const NS::Core::Vector3 origin{0.0f, 0.0f, 0.0f};
        const NS::Core::Matrix vp = MakeOrbitViewProjection(NS::Core::Vector3{6.0f, 5.0f, -7.0f}, origin);
        const NS::Core::Size2D viewport{k_ViewWidth, k_ViewHeight};

        NS::Core::Vector2 origin2d{};
        NS::Core::Vector2 grab2d{};
        ASSERT_TRUE(ProjectPoint(vp, origin, origin2d));
        ASSERT_TRUE(ProjectPoint(vp, origin + axisDir * 0.6f, grab2d));

        const GizmoAxis picked =
            GizmoEditor::ToolHandlePick(origin, NS::Core::Quaternion::Identity, GizmoTool::Move, grab2d, vp, viewport);
        EXPECT_EQ(picked, axis);

        // 掴んだ点から矢印の伸びる向きへ 60 px 引く
        NS::Core::Vector2 screenDir{grab2d.x - origin2d.x, grab2d.y - origin2d.y};
        ...
        const NS::Core::Vector3 moved =
            GizmoEditor::ComputeAxisMove(origin, axis, NS::Core::Quaternion::Identity, rayStart, rayNow, false);
        EXPECT_GT((moved - origin).Dot(axisDir), 0.1f);
    }

    // 点から線分までの画面上の距離
    float PixelDistanceToSegment(NS::Core::Vector2 p, NS::Core::Vector2 a, NS::Core::Vector2 b)
    {
        ...
        float t = 0.0f;
        if (lengthSq > 1e-6f)
            t = NS::Core::Clamp(((p.x - a.x) * abx + (p.y - a.y) * aby) / lengthSq, 0.0f, 1.0f);
    ...
    TEST(GizmoEditorArrow, SweepViewAngles)
    {
        ...
        for (int pitchDeg = 10; pitchDeg <= 80; pitchDeg += 10)
        {
            for (int yawDeg = 0; yawDeg < 360; yawDeg += 30)
            {
                ...
                const NS::Core::Vector3 eye{10.0f * std::cos(pitch) * std::sin(yaw),
                                            10.0f * std::sin(pitch),
                                            10.0f * std::cos(pitch) * std::cos(yaw)};
                const NS::Core::Matrix vp = MakeOrbitViewProjection(eye, origin);
                ...
                for (int i = 0; i < 3; ++i)
                {
                    ...
                    NS::Core::Vector2 dir{grab2d.x - origin2d.x, grab2d.y - origin2d.y};
                    ...
                    const GizmoAxis picked = GizmoEditor::ToolHandlePick(
                        origin, NS::Core::Quaternion::Identity, GizmoTool::Move, grab2d, vp, viewport);
                    ...
                    const NS::Core::Vector3 moved = GizmoEditor::ComputeAxisMove(
                        origin, axisEnum[i], NS::Core::Quaternion::Identity, rayStart, rayNow, false);
    ...
    TEST(GizmoEditorArrow, OverlappedArrowsStayReachable)
    {
        ...
        const NS::Core::Vector3 eye{0.0f, 15.0f * std::sin(pitch), -15.0f * std::cos(pitch)};
        const NS::Core::Matrix vp = MakeOrbitViewProjection(eye, origin);
        const NS::Core::Size2D viewport{k_ViewWidth, k_ViewHeight};
        ...
        NS::Core::Vector2 origin2d{};
        NS::Core::Vector2 yTip2d{};
        NS::Core::Vector2 zTip2d{};
        ASSERT_TRUE(ProjectPoint(vp, origin, origin2d));
        ASSERT_TRUE(ProjectPoint(vp, origin + NS::Core::Vector3{0.0f, handleLength, 0.0f}, yTip2d));
        ASSERT_TRUE(ProjectPoint(vp, origin + NS::Core::Vector3{0.0f, 0.0f, handleLength}, zTip2d));

        // 前提として 2 本が重なっている
        ASSERT_LT(PixelDistanceToSegment(zTip2d, origin2d, yTip2d), 4.0f);

        const NS::Core::Vector2 onZ{(origin2d.x + zTip2d.x) * 0.5f, (origin2d.y + zTip2d.y) * 0.5f};
        EXPECT_EQ(
            GizmoEditor::ToolHandlePick(origin, NS::Core::Quaternion::Identity, GizmoTool::Move, onZ, vp, viewport),
            GizmoAxis::Z);

        // Z の先端より外は Y だけが居る
        const NS::Core::Vector2 beyondZ{zTip2d.x + (yTip2d.x - zTip2d.x) * 0.7f,
                                        zTip2d.y + (yTip2d.y - zTip2d.y) * 0.7f};
        EXPECT_EQ(
            GizmoEditor::ToolHandlePick(origin, NS::Core::Quaternion::Identity, GizmoTool::Move, beyondZ, vp, viewport),
            GizmoAxis::Y);
    }

    TEST(GizmoEditorArrow, XArrowGrabsAndMoves)
    {
        ExpectArrowGrabAndDrag(GizmoAxis::X, NS::Core::Vector3{1.0f, 0.0f, 0.0f});
    }

    TEST(GizmoEditorArrow, YArrowGrabsAndMoves)
    {
        ExpectArrowGrabAndDrag(GizmoAxis::Y, NS::Core::Vector3{0.0f, 1.0f, 0.0f});
    }

    TEST(GizmoEditorArrow, ZArrowGrabsAndMoves)
    {
        ExpectArrowGrabAndDrag(GizmoAxis::Z, NS::Core::Vector3{0.0f, 0.0f, 1.0f});
    }
} // namespace
