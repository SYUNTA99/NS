...
namespace
{
    ...
    Vector3 HighEdgeCentroid(const std::array<Triangle, 8>& tris)
    {
        ...
        for (const Triangle& t : tris)
            for (const Vector3* v : {&t.v0, &t.v1, &t.v2})
        ...
        for (const Triangle& t : tris)
            for (const Vector3* v : {&t.v0, &t.v1, &t.v2})
            {
                ...
                if (!seen)
                    unique.push_back(Vector3{v->x, v->y, v->z});
            }

        Vector3 sum{0.0f, 0.0f, 0.0f};
        for (const Vector3& u : unique)
        {
            sum.x += u.x;
            sum.z += u.z;
        }
...
{
    const std::array<Triangle, 8> tris = BuildWedgeTriangles(Vector3{0.0f, 0.0f, 0.0f}, Vector3{0.5f, 0.5f, 0.5f}, 45.0f, 0.0f);
...
{
    const std::array<Triangle, 8> tris = BuildWedgeTriangles(Vector3{0.0f, 0.0f, 0.0f}, Vector3{0.5f, 0.5f, 0.5f}, 45.0f, k_HalfPi);
    const Vector3 high = HighEdgeCentroid(tris);
...
{
    const std::array<Triangle, 8> tris = BuildWedgeTriangles(Vector3{0.0f, 0.0f, 0.0f}, Vector3{0.5f, 0.5f, 0.5f}, 45.0f, k_QuarterPi);
    const Vector3 high = HighEdgeCentroid(tris);
...
{
    const std::array<Triangle, 8> t0 = BuildWedgeTriangles(Vector3{0.0f, 0.0f, 0.0f}, Vector3{0.5f, 0.5f, 0.5f}, 45.0f, 0.0f);
    const std::array<Triangle, 8> t1 = BuildWedgeTriangles(Vector3{0.0f, 0.0f, 0.0f}, Vector3{0.5f, 0.5f, 0.5f}, 45.0f, k_QuarterPi);
...
TEST(WedgeGeometryTest, CenterOffsetsAllVertices)
{
    ...
    const std::array<Triangle, 8> tris = BuildWedgeTriangles(c, Vector3{0.5f, 0.5f, 0.5f}, 45.0f, 0.0f);
