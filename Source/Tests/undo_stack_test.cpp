...
TEST(UndoStackTest, PushExecutesDoAndStoresInUndoStack)
{
    ...
    std::uint32_t next = 0;
    LevelNs::EditTarget t{lv, ids, next};
    stack.Push(std::make_unique<EditorNs::PlaceCommand>(0, 0, 0, 1, 0), t);
...
TEST(UndoStackTest, MaxOpsCapPopsOldest)
{
    ...
    {
        stack.Push(std::make_unique<EditorNs::PlaceCommand>(static_cast<std::int16_t>(i), 0, 0, 1, 0), t);
    }
...
TEST(UndoStackTest, PushAfterUndoClearsRedoStack)
{
    ...
    stack.Push(std::make_unique<EditorNs::PlaceCommand>(1, 0, 0, 2, 0), t);
    EXPECT_EQ(stack.RedoSize(), 0u);
}
