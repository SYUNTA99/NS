...
namespace NS::Editor
{
    PlaceCommand::PlaceCommand(NS::Scene::ObjectData prototype,
                               std::int16_t x,
                               std::int16_t y,
                               std::int16_t z,
    ...
    void PlaceCommand::Do(NS::Scene::SceneData& level) noexcept
    {
    ...
    void PlaceCommand::Undo(NS::Scene::SceneData& level) noexcept
    {
        const std::size_t index = NS::Game::Level::FindObjectAtCell(level, m_x, m_y, m_z);
        if (index == NS::Scene::kNoObjectIndex)
            return;
