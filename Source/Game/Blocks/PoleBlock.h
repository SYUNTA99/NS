...
///
/// @details MeshRendererComponent (cylinder mesh) + PoleComponent (trigger) を named member として保有する
/// Block (AABB collider 専用) からではなく GameObject を直接派生する
/// Mesh / Material は scene 寿命中、 共有 raw pointer として LevelEditorScene が持つ
/// 描画は per-block MeshRendererComponent::Draw 経路 (SlopeBlock と同じ、 InstanceBatcher は cube 専用)
...
{
public:
    PoleBlock(NS::Graphics::StaticMesh* cylinderMesh, NS::Graphics::Material* material, float radius, float height) noexcept;
