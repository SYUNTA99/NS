...
namespace NS::Game::Player
{
    ...
    //! 優先度は Update 帯の +50。移動が終わった後に選び、骨を回す SkeletalAnimation (+100) より前に置く
    //! 依存: PlayerComponent, PlayerStateManager, NS::Obj::SkeletalAnimation
    class PlayerAnimator : public NS::Obj::Component
    {
    public:
        PlayerAnimator() noexcept;

        //! 同居する PlayerComponent / PlayerStateManager / SkeletalAnimation を控える
        void OnStart() override;
        //! クリップと再生速度を選び直す。PlayerComponent か SkeletalAnimation が欠けていれば何もしない
        ...
        NS_REFLECT_BEGIN(PlayerAnimator, NS::Obj::Component)
    ...
    private:
        ...
        PlayerStateManager* m_states = nullptr;               // 現在状態の問い合わせ先 (非所有)
        NS::Obj::SkeletalAnimation* m_animation = nullptr; // クリップの差し先 (非所有)
