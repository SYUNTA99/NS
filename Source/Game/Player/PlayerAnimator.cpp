...
{
    // 移動 (Update) と骨を回す SkeletalAnimation (Update + 100) の間ならどこでもよい
    // 真ん中に置いて両側へ足せる幅を残す
    PlayerAnimator::PlayerAnimator() noexcept
        : NS::Obj::Component(NS::Obj::TickPriority::Update + 50)
    {}

    void PlayerAnimator::OnStart()
    {
        ...
        m_states = Owner()->FindComponent<PlayerStateManager>();
        m_animation = Owner()->FindComponent<NS::Obj::SkeletalAnimation>();
    }
    ...
    std::string_view PlayerAnimator::ChooseClip(float lateralSpeed) const noexcept
    {
    ...
    float PlayerAnimator::ChoosePlaybackSpeed(std::string_view clip, float lateralSpeed) const noexcept
    {
        // 速さに合わせるのは足が地面を蹴る絵だけ
