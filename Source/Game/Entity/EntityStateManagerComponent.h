...
namespace NS::Game::Entity
{
    ...
    //! @details 所有者の型が付いた NS::Obj::StateMachine は派生が持つ。ここにあるのは、所有者の型を
    //! 知らずに状態の型で移す・問う口だけ
    //! TypeRegistry には登録しない。実体化できるのは派生だけ
    //! 依存: NS::Obj::Component
    class EntityStateManagerComponent : public NS::Obj::Component
    {
    public:
        ...
        NS_REFLECT_NONE(EntityStateManagerComponent, NS::Obj::Component)

    protected:
        //! 印の状態へ移る。並びに無い状態と未組立は false
        //! @details 外から呼べるのは Change だけにする。StateId は const void*
        //! なので、状態でない番地を渡してもビルドが通る
        virtual bool ChangeToState(NS::Obj::StateId id) = 0;

        //! 現在状態の印。未組立は nullptr
        [[nodiscard]] virtual NS::Obj::StateId CurrentStateId() const noexcept = 0;
    };
} // namespace NS::Game::Entity
